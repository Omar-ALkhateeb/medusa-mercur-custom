  ...
  async uploadImages(imageUrls: string[]): Promise<number[]> {
    ...
    for (const imageUrl of imageUrls) {
      try {
        ...
        const filename =
          urlParts[urlParts.length - 1] || `image-${Date.now()}.jpg`;
        ...
        this.logger_.error(
          this.formatStrapiError(error, `Failed to upload image ${imageUrl}`),
        );
      }
    }
  ...
  async deleteImage(imageId: number): Promise<void> {
    try {
      ...
      throw new MedusaError(
        ...
        this.formatStrapiError(
          error,
          `Failed to delete image ${imageId} from Strapi`,
        ),
      );
    }
  }
  ...
  async update(
    ...
    try {
      ...
      throw new MedusaError(
        ...
        this.formatStrapiError(
          error,
          `Failed to update ${collection} in Strapi`,
        ),
      );
    }
  }
  ...
  async delete(collection: Collection, id: string) {
    try {
      ...
      throw new MedusaError(
        ...
        this.formatStrapiError(
          error,
          `Failed to delete ${collection} in Strapi`,
        ),
      );
    }
  }
    ...
    for (const productId of ids) {
      try {
        ...
        if (result.data && result.data.length > 0) {
          ...
          results.push({
            ...
            variants: (product.variants || []).map((variant: any) => ({
              ...variant,
              id: `${variant.id}`,
              option_values: (variant.option_values || []).map(
                (option_value: any) => ({
                  ...option_value,
                  id: `${option_value.id}`,
                }),
            ...
            collection: product.collection
              ? { ...product.collection, id: `${product.collection.id}` }
              : null,
        ...
        this.logger_.warn(
          this.formatStrapiError(
            error,
            `Failed to fetch product ${productId} from Strapi`,
          ),
        );
      }
    }
