...
export const updateOptionInStrapiWorkflow = createWorkflow(
  ...
  (input: UpdateOptionInStrapiWorkflowInput) => {
    ...
    const { data: options } = useQueryGraphStep({
      ...
      fields: ["id", "title", "product.strapi_product.*", "values.*"],
      filters: {
        id: input.id,
      },
      options: {
    ...
    const optionValuesToCreate = transform(
      ...
      (data) => {
        ...
        // @ts-ignore
        return data.options[0].values
          .filter((value) => !value.metadata?.strapi_id)
          .map((value) => {
            return {
              id: value.id,
              value: value.value,
    ...
    when(
      { updateResult, optionValuesToCreate },
      (data) => !!data.updateResult && !!data.optionValuesToCreate.length,
    ).then(() => {
      const strapiOptionValues = createOptionValuesInStrapiStep({
        optionValues: optionValuesToCreate,
      });

      const optionValuesMetadataUpdate = transform(
        { strapiOptionValues },
        (data) => {
          return {
            updates: [
    ...
    const optionValuesToUpdate = transform(
      { options, updateResult },
      (data) => {
        if (!data.updateResult) {
          return [];
    ...
    when(
      { updateResult, optionValuesToUpdate },
      (data) => !!data.updateResult && !!data.optionValuesToUpdate.length,
    ).then(() => {
    ...
    const optionValuesToDelete = transform(
      { options, updateResult },
      (data) => {
        if (!data.updateResult) {
          return [];
        }
        return (data.options[0].product as any)?.strapi_product?.options
          ?.find((option) => option.medusaId === data.options[0].id)
          ?.values.filter(
            (value) =>
              !data.options[0].values.some((v) => v.id === value.medusaId),
          )
          .map((value) => {
    ...
    when(
      { updateResult, optionValuesToDelete },
      (data) => !!data.updateResult && !!data.optionValuesToDelete.length,
    ).then(() => {
