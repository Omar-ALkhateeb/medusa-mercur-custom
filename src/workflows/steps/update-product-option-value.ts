...
export const updateProductOptionValueStep = createStep(
  ...
  async ({ id, value }: UpdateProductOptionValueInput, { container }) => {
    ...
    const oldOptionValue =
      await productModuleService.retrieveProductOptionValue(id);

    // Update the option value
    const updatedOptionValue =
      await productModuleService.updateProductOptionValues(id, {
        value,
      });
  ...
  async (compensateData, { container }) => {
    ...
    await productModuleService.updateProductOptionValues(compensateData.id, {
