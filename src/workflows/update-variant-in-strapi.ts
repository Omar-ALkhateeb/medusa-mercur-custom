...
import {
  UpdateVariantInStrapiInput,
  updateVariantInStrapiStep,
} from "./steps/update-variant-in-strapi";
...
export const updateVariantInStrapiWorkflow = createWorkflow(
  ...
  (input: UpdateVariantInStrapiWorkflowInput) => {
    ...
    const createResult = when(
      { variants },
      (data) => !data.variants[0].metadata?.strapi_id,
    ).then(() => {
    ...
    const updateResult = when(
      ...
      const { variantImages, variantThumbnail } = transform(
        { variants },
        (data) => {
          return {
            variantImages:
        ...
        (data) => {
          return {
            ...
            optionValueIds: data.variants[0].options.flatMap((option) => {
              ...
              return data.variants[0].product?.options
                .flatMap(
                  (productOption) =>
